locals {
  # Get list of domains and SANs
  distinct_domain_names = distinct(
    [for s in concat([var.domain_name], var.subject_alternative_names) : replace(s, "*.", "")]
  )

  # Get list of domain_validation_options, wildcard domains
  # are replaced by the domain name
...
resource "aws_route53_record" "validation" {
  count = var.validation_method == "DNS" && var.create_route53_records && var.validate_certificate ? length(local.distinct_domain_names) : 0
